...
var CppReserved = []string{
	"char",
}

//This is the C compiler for uct.
var Cpp bool

func init() {
	flag.BoolVar(&Cpp, "c++", false, "Target the C programming language")

	RegisterAssembler(CppAssembly, &Cpp, "cpp", "//")

	for _, word := range CppReserved {
		CppAssembly[word] = Reserved()
	}
}

var CppAssembly = Assemblable{
	//Special commands.
	"HEADER": Instruction{
		Data:   `
//System configiration.
	...
	//You need to implement these for your system.
	void myputchar(char c) { putchar(c); }
	void myexit(int code) { exit(code); }
	void myerror() { printf("[ERROR] Out of memory!"); }
//End configuriation.
...
int stack_get() {
  ...
  if (index < (BLOCK_SIZE - data_pos)) {
    return activearray->index[length+data_pos];
  } else {
...
void stack_stdout() {
  ...
  for (int i = 0; i < arr->index[length_pos]; i++) {
    if (i < (BLOCK_SIZE - data_pos)) {
      myputchar(arr->index[i+data_pos]);
    } else {
       myputchar(*(stack_get_array_segment(arr, i)));
    }
  }
